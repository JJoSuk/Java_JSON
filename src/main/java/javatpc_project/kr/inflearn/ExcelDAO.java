package javatpc_project.kr.inflearn;
import java.io.*;
import java.net.*;
import java.util.*;
...
public class ExcelDAO {
    ...
    public void excel_input() {
    	...
    	try {
            ...
            cellA.setCellValue(new HSSFRichTextString("책 제목"));
            ...
            cellE.setCellValue(new HSSFRichTextString("이미지 이름"));
            ...
            while(true) {
	            ...
	            list.add(data);
	            System.out.print("계속 입력하시면 Y / 입력 종료 N : ");
	...
	//상세 검색은 책 제목(d_titl), 저자명(d_auth), 목차(d_cont), ISBN(d_isbn), 출판사(d_publ) 5개 항목 중에서 1개 이상 값을 입력야해야함.
public ExcelVO naver_search(ExcelVO vo) {
		try {
            ...
            String inputLine;
            StringBuffer response = new StringBuffer(); // 문자열 추가 변경 시 사용
