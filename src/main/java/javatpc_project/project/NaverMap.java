...
import javatpc_project.kr.inflearn.AddressVO;
...
public class NaverMap implements ActionListener{
    ...
    public void actionPerformed(ActionEvent e) {
        String clientId = "8vdvnglhiu";
        String clientSecret = "Wf7VI5JCHlXc7EqcN3IoiUu8bwcnkEqnoOnAJeAv";
        AddressVO vo=null;
        try {
            String address =naverMap.address.getText();
            String addr = URLEncoder.encode(address, "UTF-8"); // 입력 공백도 문자처리 해줘야 한다.
            ...
            for(int i=0;i<arr.length();i++){
                ...
                vo = new AddressVO(
                        (String)temp.get("roadAddress"),
                        (String)temp.get("jibunAddress"),
                        (String)temp.get("x"),
                        (String)temp.get("y"));
    ...
    public void map_service(AddressVO vo) {
        ...
        try {
            ...
            HttpURLConnection con = (HttpURLConnection)url.openConnection();
            con.setRequestMethod("GET");
            con.setRequestProperty("X-NCP-APIGW-API-KEY-ID", "8vdvnglhiu");
            con.setRequestProperty("X-NCP-APIGW-API-KEY", "Wf7VI5JCHlXc7EqcN3IoiUu8bwcnkEqnoOnAJeAv");
            int responseCode = con.getResponseCode();
            BufferedReader br;
